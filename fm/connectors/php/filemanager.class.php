...
class Filemanager {
	...
	public function add() {
		$textarea = (isset($_POST['textarea']))  ? filter_var($_POST['textarea'], FILTER_VALIDATE_BOOLEAN) : true;
		...
		if(!isset($_FILES['newfile']) || !is_uploaded_file($_FILES['newfile']['tmp_name'])) {
			...
			if($this->config['upload']['fileSizeLimit'] > $this->getMaxUploadFileSize()) {
				...
				$this->config['upload']['fileSizeLimit'] = $this->getMaxUploadFileSize();
				$this->error(sprintf($this->lang('UPLOAD_FILES_SMALLER_THAN'),$this->config['upload']['fileSizeLimit'] . $this->lang('mb')),$textarea);
			}
			
			$this->error(sprintf($this->lang('INVALID_FILE_UPLOAD') . ' '. sprintf($this->lang('UPLOAD_FILES_SMALLER_THAN'),$this->config['upload']['fileSizeLimit'] . $this->lang('mb'))),$textarea);
		}
		// we determine max upload size if not set
		if($this->config['upload']['fileSizeLimit'] == 'auto') {
			$this->config['upload']['fileSizeLimit'] = $this->getMaxUploadFileSize();
		...
		if(!$this->is_allowed_file_type($_FILES['newfile']['name'])) {
			$this->error(sprintf($this->lang('INVALID_FILE_TYPE')), $textarea);
		}
		...
		if($textarea) echo '<textarea>' . json_encode($response) . '</textarea>';
